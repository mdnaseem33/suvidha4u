...
import com.martvalley.suvidha_u.databinding.ActivityAntiReverseBinding
...
class AntiReverseActivity : AppCompatActivity() {
    val binding by lazy { ActivityAntiReverseBinding.inflate(layoutInflater) }
    private val id by lazy { intent.getIntExtra("id", 0) }
    private val amount by lazy { intent.getIntExtra("amount", 0) }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(binding.root)
        binding.balanceTv.text = "Available Balance: ${amount}"
    ...
    private fun callCreditApi(amt: String, description: String?=null) {
        binding.pb.show()
        val request = Retailer.AntiReverseRetailerRequest(amt, id.toString(), description)
        val call = RetrofitInstance.apiService.antiReverseRetailerApi(request)
